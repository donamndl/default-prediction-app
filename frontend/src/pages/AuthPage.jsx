...
const IcoEyeOpen = () => (
  <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5">
    <path strokeLinecap="round" strokeLinejoin="round"
      d="M2.458 10C3.732 6.943 6.523 5 10 5c3.478 0 6.268 1.943 7.542 5-1.274 3.057-4.064 5-7.542 5-3.477 0-6.268-1.943-7.542-5z"/>
...
const IcoEyeClosed = () => (
  <svg viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5">
    <path strokeLinecap="round" strokeLinejoin="round"
      d="M3.98 8.223A10.477 10.477 0 001.934 10c1.292 3.068 4.04 5.25 7.5 5.25 1.17 0 2.29-.22 3.32-.617m3.197-2.796A10.33 10.33 0 0018.066 10c-1.292-3.068-4.04-5.25-7.5-5.25a10.357 10.357 0 00-3.38.564M3 3l14 14"/>
  </svg>
)
...
───────────────────────────────────────────── */
const PasswordInput = ({ value, onChange, placeholder, error, autoComplete = 'current-password' }) => {
  ...
  return (
    <div className="lp-input-wrap">
      ...
      <input
        ...
        className={`lp-input lp-input-has-eye ${error ? 'lp-input-error' : ''}`}
      />
      <button
        type="button"
    ...
    <form className="modal-form" onSubmit={handleSubmit} noValidate>
      ...
      <div className="modal-fields">
        ...
        <div className="lp-field">
          ...
          {errors.password && <span className="lp-field-error">⚠ {errors.password}</span>}
          {password.length > 0 && (
            <div className="modal-strength" style={{ marginTop: 6 }}>
      ...
      <div className="modal-box animate-scale-in">
        ...
        <div className="modal-tabs">
          {['login','register'].map(m => (
            <button key={m} type="button" className={`modal-tab ${mode===m?'modal-tab-active':''}`} onClick={()=>setMode(m)}>
...
const ContactForm = () => {
  ...
  if (sent) return (
    <div className="contact-success">
      ...
      <button className="contact-reset-btn" onClick={() => { setSent(false); setForm({name:'',email:'',subject:'',message:''}) }}>Send another</button>
    </div>
  )
...
const HOW_IT_WORKS = [
  {num:'01',title:'Fill the 7-Step Form',desc:'Applicant details collected across seven structured sections — family, residence, employment, banking, credit bureau, health, and compliance.'},
  {num:'02',title:'Scorecard + ML Analysis',desc:'Your 350-point scorecard runs instantly. The trained ML model simultaneously predicts default probability from all 51 variables.'},
...
const TYPEWRITER_PHRASES = ['faster decisions.','smarter approvals.','zero guesswork.','instant STP.','real-time risk.','full auditability.']
    ...
    <div className="lp-root">
      ...
      <section className="lp-hero" id="about">
        ...
        <div className="lp-hero-inner">
          <div className="lp-hero-badge"><span className="lp-badge-dot"/>V2.4 AI Risk Engine Live</div>
          <h1 className="lp-hero-h1">
            <span className="lp-hero-line1">Precision lending. Intelligent decisions.</span>
          ...
          <p className="lp-hero-p">Deploy high-performance credit models in minutes. Our proprietary 350-point scorecard uses predictive ML to automate risk assessment for modern lenders — faster, smarter, and fully auditable.</p>
          ...
          <div className="lp-stats-row">
            {[{v:'350',l:'Scorecard Points'},{v:'51',l:'Risk Variables'},{v:'4',l:'Approval Levels'},{v:'7',l:'Form Sections'}].map(s=>(
              <div key={s.l} className="lp-stat"><div className="lp-stat-v">{s.v}</div><div className="lp-stat-l">{s.l}</div></div>
            ))}
          </div>
          ...
          <div className="lp-footer-info-cols">
            ...
            <div className="lp-footer-col">
              ...
              <div className="lp-socials">
                <a href="https://linkedin.com" target="_blank" rel="noreferrer" className="lp-social" title="LinkedIn"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg></a>
                <a href="https://instagram.com" target="_blank" rel="noreferrer" className="lp-social" title="Instagram"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg></a>
                <a href="https://youtube.com" target="_blank" rel="noreferrer" className="lp-social" title="YouTube"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M23.495 6.205a3.007 3.007 0 00-2.088-2.088c-1.87-.501-9.396-.501-9.396-.501s-7.507-.01-9.396.501A3.007 3.007 0 00.527 6.205a31.247 31.247 0 00-.522 5.805 31.247 31.247 0 00.522 5.783 3.007 3.007 0 002.088 2.088c1.868.502 9.396.502 9.396.502s7.506 0 9.396-.502a3.007 3.007 0 002.088-2.088 31.247 31.247 0 00.5-5.783 31.247 31.247 0 00-.5-5.805zM9.609 15.601V8.408l6.264 3.602z"/></svg></a>
                <a href="mailto:contact@creditsense.io" className="lp-social" title="Email"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M24 5.457v13.909c0 .904-.732 1.636-1.636 1.636h-3.819V11.73L12 16.64l-6.545-4.91v9.273H1.636A1.636 1.636 0 010 19.366V5.457c0-2.023 2.309-3.178 3.927-1.964L5.455 4.64 12 9.548l6.545-4.91 1.528-1.145C21.69 2.28 24 3.434 24 5.457z"/></svg></a>
                <a href="https://github.com" target="_blank" rel="noreferrer" className="lp-social" title="GitHub"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/></svg></a>
              </div>
            </div>
          </div>
        </div>
